...
void afficheSimulation2(sdl2::window& écran, std::vactor<int> valuesGrippe, std::vector<int> valuesAgent, int largeur_grille, int hauteur_grille)
{
    auto [largeur_écran,hauteur_écran] = écran.dimensions();
    sdl2::font fonte_texte("./graphisme/src/data/Lato-Thin.ttf", 18);
    écran.cls({0x00,0x00,0x00});
...
void simulation(bool affiche)
{
	...
	while (!quitting)
	{
		...
		int largeur_grille;
		int hauteur_grille;
		MPI_Irecv(&largeur_grille, 1, MPI_INT, 100, MPI_COMM_WORLD, request);
		MPI_Irecv(&hauteur_grille, 1, MPI_INT, 101, MPI_COMM_WORLD, request);
		std::vector<int> valuesGrippe(largeur_grille*hauteur_grille);
		std::vector<int> valuesAgent(largeur_grille*hauteur_grille);
		while (1){
			MPI_Recv(valuesAgent.data(),largeur_grille*hauteur_grille, 1, MPI_INT, 201, MPI_COMM_WORLD, status);
			MPI_Recv(valuesGrippe.data(),largeur_grille*hauteur_grille, 1, MPI_INT, 200, MPI_COMM_WORLD, status);
			if (affiche) afficheSimulation2(écran, valuesGrippe, valuesAgent, largeur_grille, hauteur_grille);
		}
